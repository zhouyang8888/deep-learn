...
public:
    ...
    inline mem_block(void* start, uint32_t len, uint32_t sn=0) : start(start), len(len), sn(sn) {}
...
{
public:
    inline mem_block2(void* start, uint32_t len, uint32_t sn=0) : mem_block(start, len, sn) {}
